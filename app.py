...
# Reuse limits with ranges and symbols
limits = {
    'Turbidity_final_NTU': ('< 5.0', 5.0),
    'Fe_final_mg_L': ('< 0.3', 0.3),
    'Mn_final_mg_L': ('< 0.1', 0.1),
    'Cu_final_mg_L': ('< 1.0', 1.0),
    'Zn_final_mg_L': ('< 5.0', 5.0),
    'Suspended_solids_final_mg_L': ('< 50.0', 50.0),
    'TDS_final_mg_L': ('< 1000.0', 1000.0)
}

results = None  # global placeholder
...
if submitted:
    ...
    results['Standard Limit'] = ['< 5.0', '< 0.3', '< 0.1', '< 1.0', '< 5.0', '< 50.0', '< 1000.0', '', '', '']
    ...
    for i, var in enumerate(output_vars[:7]):
        value = y_pred[i]
        if value <= limits[var][1]:
            assessment.append("✅ OK")
        else:
            assessment.append("❌ Exceeds")
            reuse_safe = False
    assessment += ["--"] * 3
...
def create_pdf(results_df):
    ...
    pdf.cell(190, 10, "Water Quality Prediction Report", ln=1, align="C")
    pdf.set_font("Arial", "", 12)

    for i in range(len(results_df)):
        row = results_df.iloc[i]
        pdf.cell(0, 10, f"{row.name}: {row['Predicted Value']:.3f} {row['Unit']} | Standard: {row['Standard Limit']} | {row['Assessment']}", ln=1)

    return pdf.output(dest="S").encode("latin1")

# Show the download button at the bottom
st.markdown("---")
...
if results is not None:
    ...
    st.info("Submit test results first to enable PDF download.")
